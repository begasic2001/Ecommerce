...
export const Search = styled.section`
  ...
  padding: 0.25rem;
  border: ${({ isShowSearch, theme: { colors } }) =>
    isShowSearch ? `solid 1px ${colors.border}` : 'none'};
  width: ${({ isShowSearch }) => (isShowSearch ? '25rem' : '4rem')};
  height: 4rem;
  background-color: #fff;
...
export const UserLogBox = styled.section`
  ...
  min-width: 12rem;
  background-color: #fff;
  opacity: ${({ isOpenUserLog }) => (isOpenUserLog ? 1 : 0)};
  transform: ${({ isOpenUserLog }) => (isOpenUserLog ? 'scale(1)' : 'scale(0)')};
